...
- Правильно: animal-details.component.tsx, create-animal.command.ts
- Неправильно: AnimalDetails.tsx, createAnimalDto.ts
...
Используем стандартные `enum` или `const assertions`.
...
### 3.4. Mappers and Utilities

- Чистая логика (без побочных эффектов) и преобразование данных (например, генерация URL, форматирование строк) выносятся в классы-мапперы (например, `AppUrlMapper`).
- Мапперы должны содержать только **статические методы**.
- Мапперы ЗАПРЕЩЕНО помечать как `@Injectable` или использовать в DI. Все внешние данные маппер должен принимать как аргументы метода.
