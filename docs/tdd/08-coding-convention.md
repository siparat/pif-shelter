...
По умолчанию мы используем kebab-case для всех файлов и папок (это избавляет от проблем с чувствительностью к регистру в разных ОС и применяется во всём backend-коде):

- Правильно: `create-animal.command.ts`, `animal.repository.ts`
- Неправильно: `createAnimalDto.ts`

**Исключение для фронтенда (см. ADR-014):** файлы и папки внутри `ui/`, содержащие JSX/TSX-компоненты, пишутся в **PascalCase** (`AnimalCard.tsx`, `AnimalsPage/`). Хуки, утилиты, конфиги и стили во фронтенде остаются в kebab-case.
