...
Исключения из envelope допустимы только для следующих случаев:

1. Redirect-эндпоинты (`3xx`) — например выдача временной ссылки на внешний ресурс (S3 presigned URL).
2. Binary/stream ответы (`application/pdf`, `application/octet-stream`, `xlsx`) — когда endpoint отдает файл напрямую.
3. Совместимость с внешними webhook/protocol контрактами, где формат строго задан провайдером.

Для таких endpoint обязательно явно документировать статус/формат в Swagger и не смешивать их с JSON-envelope.
