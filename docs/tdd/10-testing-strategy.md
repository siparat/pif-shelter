...
### 1.3. Тесты Policy (Проверка правил доступа)

Policy-классы, которые проверяют права перед выполнением команды (например, `CanEditAnimalPolicy`, `StartGuardianshipPolicy`), тестируются юнит-тестами с моками репозиториев и сервисов. Покрываются сценарии: успешный доступ, отказ по отсутствию сущности, отказ по недостатку прав (роль, владение). Файл: `<module>/policies/<name>.policy.spec.ts`.
...
- Конкретный файл: `nx test api` с опцией выбора файла в конфигурации Jest (см. документацию Nx/Jest для актуального синтаксиса).

## 5. Почему это лучше контроллеров?
