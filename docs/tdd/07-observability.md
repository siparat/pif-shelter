...
Метрики Prometheus планируются (отдельный модуль ещё не подключён). При интеграции приложение должно выставлять метрики по адресу `/metrics`, защищённому Basic Auth или доступному только во внутренней сети Docker.
