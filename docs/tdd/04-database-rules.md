...
- Упорядоченные списки медиа (например, галерея поста): предпочтительно отдельная таблица с полем `order` (smallint/integer) для однозначной сортировки и расширяемости; допустим упорядоченный jsonb-массив ключей, если порядок не требует отдельной сущности.
...
2. Запускает `nx generate database`.
3. Drizzle Kit создает SQL файл миграции в `/migrations`.
4. Разработчик проверяет SQL файл.
5. Миграции применяются при деплое командой `nx migrate database` (в Docker — отдельный сервис `migrate` из `docker-compose.yml`).
...
## 7. Артефакты отчетов (Async Generated Artifacts)

Для асинхронно генерируемых отчетов (например, месячный XLSX, PDF) допускается отдельная таблица артефактов со статусным жизненным циклом.

Рекомендуемые поля:

1. Период/бизнес-ключ (`year`, `month` или эквивалент).
2. Тип артефакта (`report_type`) — для поддержки нескольких форматов.
3. Статус генерации (`PENDING`, `SUCCEEDED`, `FAILED`).
4. `storage_key` (только относительный ключ в S3, без URL).
5. `checksum_sha256` для контроля целостности и сравнения регенераций.
6. `generated_at` и `error_message` для операционной диагностики.

Обязательно:

- Уникальный индекс по периоду и типу (например `year + month + report_type`) для идемпотентности.
- Не хранить presigned/public URL в БД — URL формируется на чтении.
