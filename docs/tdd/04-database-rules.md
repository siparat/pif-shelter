...
- Files & Images -> text. Храним только **относительный путь** (key) внутри бакета (например, `avatars/user-1.webp`). Запрещено хранить полные URL с доменом.
...
2. Создавать Админа (admin@pif.xyz / admin) — данные из .env (`ADMIN_EMAIL`, `ADMIN_PASSWORD`, `ADMIN_TELEGRAM`).
3. Создавать набор справочников (категории товаров).
4. Создавать тестовых животных (10 шт) и волонтеров.
5. Записывать контент для Telegram-бота (например, тексты для команды `/help`: контакты, адрес, правила посещения) из переменных окружения (например `BOT_HELP_*`) в таблицу конфигурации/контента, чтобы не хардкодить их в коде.
