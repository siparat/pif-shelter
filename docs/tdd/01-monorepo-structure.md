...
* **Пакетный менеджер:** `pnpm`. Выбран за эффективность использования диска (hard links) и скорость установки.
* **Workspaces:** Включены в конфигурации.
...
**Паттерн работы:**
1.  Все контракты создаются в библиотеке `libs/shared/dto`.
2.  Файл контракта экспортирует два артефакта:
    * **Schema (Runtime):** Zod-схема для валидации данных во время выполнения.
    * **Type (Static):** TypeScript-тип, выведенный из схемы (`z.infer<T>`).
3.  **Backend Usage:**
