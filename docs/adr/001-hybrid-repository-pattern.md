...
- **Decision:**
  Мы принимаем **Гибридный подход** разделения ответственности:
    1.  **Write Side (Commands):**
        ...
        - _Причина:_ Здесь важна инкапсуляция бизнес-правил и целостность агрегатов.

    2.  **Read Side (Queries):**
        - Отказываемся от репозиториев.
        - Query Handlers имеют право инжектить `DatabaseService` напрямую.
        - Запросы возвращают сразу готовые DTO (Projections), минуя Domain Entities.
        - _Причина:_ Для чтения важна производительность и гибкость выборки. Чтение не меняет состояние системы, поэтому инварианты домена здесь проверять не нужно.
...
- **Compliance:**
    - Код-ревьюверы должны отклонять PR, где в CommandHandler используется прямой `db.client.select()`.
    - Код-ревьюверы должны отклонять PR, где в QueryHandler происходит `db.client.insert/update`.
